...
REQUESTS_DAYS_NUMBER = 3
REQUESTS_HOURS_NUMBER = 72
CLEANING_DAYS_NUMBER = 90

if ENV.exists():
    UNDER_CONSTRUCTION = int(getenv('UNDER_CONSTRUCTION', UNDER_CONSTRUCTION))
    SECRET_KEY = getenv('SECRET_KEY', SECRET_KEY)
    TOKEN = getenv('TOKEN', TOKEN)
    ACCOUNT = getenv('ACCOUNT', ACCOUNT)
    PARTITION = getenv('PARTITION', PARTITION)
    USE_OLD_SUBMITER = int(getenv('USE_OLD_SUBMITER', USE_OLD_SUBMITER))

    LOGIN_NODE_URLS = getenv('LOGIN_NODE_URLS', LOGIN_NODE_URLS)
    USER_NAME = getenv('USER_NAME', USER_NAME)
    USER_ID = getenv('USER_ID', USER_ID)
    USER_PASSWORD = getenv('USER_PASSWORD', USER_PASSWORD)
    ADMIN_EMAIL = getenv('ADMIN_EMAIL', ADMIN_EMAIL)
    SMTP_SERVER = getenv('SMTP_SERVER', SMTP_SERVER)
    SMTP_PORT = int(getenv('SMTP_PORT', SMTP_PORT))
    REPORT_RECEIVERS = getenv('REPORT_RECEIVERS', REPORT_RECEIVERS).split()

    DEV_EMAIL = getenv('DEV_EMAIL', DEV_EMAIL)
    ADMIN_USER_NAME = getenv('ADMIN_USER_NAME', ADMIN_USER_NAME)
    ADMIN_PASSWORD = getenv('ADMIN_PASSWORD', ADMIN_PASSWORD)
    SEND_EMAIL_DIR_IBIS = getenv('SEND_EMAIL_DIR_IBIS', SEND_EMAIL_DIR_IBIS)
    OWNER_EMAIL = getenv('OWNER_EMAIL', OWNER_EMAIL)
    REQUEST_WAITING_TIME = int(getenv('REQUEST_WAITING_TIME', REQUEST_WAITING_TIME))
    REQUESTS_DAYS_NUMBER = int(getenv('REQUESTS_DAYS_NUMBER', REQUESTS_DAYS_NUMBER))
    REQUESTS_HOURS_NUMBER = int(getenv('REQUESTS_HOURS_NUMBER', REQUESTS_HOURS_NUMBER))
    CLEANING_DAYS_NUMBER = int(getenv('CLEANING_DAYS_NUMBER', CLEANING_DAYS_NUMBER))

REQUESTS_NUMBER = max((REQUESTS_DAYS_NUMBER * 24), REQUESTS_HOURS_NUMBER) * 60 * 60 / REQUEST_WAITING_TIME
