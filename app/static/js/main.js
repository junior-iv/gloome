...
function convertJSONToTableFoFileList(jsonData) {
    ...
    Object.entries(jsonData).forEach(([key, value]) => {
        ...
        firstRow += `<th  class="h-auto w-auto text-center toast-body alert-light bg-opacity-50">${value[0]}</th>`;
        secondRow += `<th class="h-auto w-auto text-center toast-body btn btn-light bg-opacity-50">${value[1]}</th>`;
    });
